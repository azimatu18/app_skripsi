...
<a href="/admin/validasi_dokumen" class="btn btn-outline-primary">
    ...
    <div class="mb-4">
        <div class="d-flex align-items-end gap-3 flex-wrap">
            ...
            <!-- Form Tolak Validasi Dokumen -->
            <form action="<?= base_url('admin/validasi_dokumen/diTolak/' . $dokumen['id']) ?>" method="post">
                <?= csrf_field() ?>
                <div class="mb-3">
                    <label for="alasan_penolakan" class="form-label fw-semibold">Alasan Penolakan</label>
                    <textarea name="alasan_penolakan" id="alasan_penolakan" class="form-control" rows="2" required
                        oninvalid="this.setCustomValidity('Harap isi alasan penolakan')"
                        oninput="this.setCustomValidity('')"></textarea>
                </div>
                <button type="submit" class="btn btn-danger">
                    <i class="bi bi-x-circle me-1"></i> Tolak Validasi
                </button>
            </form>

            <!-- Form Setujui Validasi Dokumen -->
            <form action="<?= base_url('admin/validasi_dokumen/diSetujui/' . $dokumen['id']) ?>" method="post">
                <?= csrf_field() ?>
                <button type="submit" class="btn btn-success">
                    <i class="bi bi-check2-circle me-1"></i> Setujui Validasi
                </button>
            </form>
